...
st.subheader("1 - Qual é a média de idade dos alunos na escola GP e MS?")
school_med_age = data.groupby('school')['age'].mean()
fig, ax = plt.subplots()
if graph1_type == "Pizza":
    ax.pie(school_med_age.values, labels=school_med_age.index, autopct='%1.1f%%')
    ax.set_title('Distribuição da Idade por Escola')
else:
    sns.barplot(x=school_med_age.index, y=school_med_age.values)
    ax.set_xlabel('Escola')
    ax.set_ylabel('Media de Idade dos estudantes')
st.pyplot(fig)


# 2. Qual é a moda do endereço dos alunos na escola MS?
st.subheader("2 - Qual é a moda do endereço dos alunos na escola MS?")
school_mode_address = data.loc[data['school'] == 'MS']['address'].mode()
match school_mode_address.values[0]:
    case 'U':
        st.write("Urbano")
    case 'R':
        st.write("Rural")
...
school_median_traveltime = data.loc[data['school'] == 'GP']['traveltime'].median()
...
school_std_age = data.loc[(data['school'] == 'MS') & (data['schoolsup'] == 'yes')]['age'].std()
...
school_mean_studytime = data.loc[(data['school'] == 'GP') & (data['Pstatus'] == 'A')]['studytime'].mean()
...
school_moda_reason = data.loc[data['school'] == 'MS']['reason'].mode()
...
school_median_absences = data.loc[data['school'] == 'GP']['absences'].median()
...
school_std_health = data.loc[(data['school'] == 'MS') & (data['activities'] == 'yes')]['health'].std()
...
school_count_activities = data.loc[data['activities'] == 'yes']['activities'].count()
...
school_moda_workday_alcohol = data.loc[data['school']=='MS']['Dalc'].mode()
